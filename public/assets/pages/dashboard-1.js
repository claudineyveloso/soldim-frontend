...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const hddChart = new Chart(document.getElementById("_dm-hddChart"), {
    type: "line",
    data: {
      datasets: [
        {
  ...
  const earningChart = new Chart(document.getElementById("_dm-earningChart"), {
    type: "line",
    data: {
      datasets: [
        {
          ...
          callbacks: {
            label: (context) => {
              ...
              if (context.parsed.x !== null)
                label += " " + context.parsed.x + " Items Completed";
              return label;
            },
          },
        },
...
["change.nf.colormode", "scheme-changed"].forEach((ev) =>
  document.addEventListener(ev, updateDashboardChart),
);
...
let getSecondaryColor = (rgb) => {
  ...
  return hslToHex(
    ...
    l > 70 ? Math.round(f ? c / f : 0 * 100) + 30 : 15,
    l > 70 ? 0.9 : 0.25,
  );
};
